...
p1.pups.log.v2 <- function(legend = TRUE){
...
for (i in 1:nEc){
  ...
  # Jitter to avoid overlap
  e <- rnorm(1, 0, 1)
  points(dsub$days+e+75, log(dsub$OD), pch = 2, 
         col = colors.Ec[i %% length(colors.Ec)],
         cex = pcex)
  if (nrow(dsub) > 0){
    lines(dsub$days+e+75, log(dsub$OD), 
          col = colors.Ec[i %% length(colors.Ec)], 
          lty=2, lwd = plwd)
  }
}
...
for (i in 1:nEcA){
  ...
  # Jitter to avoid overlap
  e <- rnorm(1, 0, 1)
  points(dsub$days+e+75, log(dsub$OD), pch = 1, 
         col = colors.EcA[i %% length(colors.EcA)], 
         cex = pcex)
  if (nrow(dsub) > 0){
    lines(dsub$days+e+75, log(dsub$OD), 
...
if(legend){
  legend(x=30,y=-2, legend = c("EcA", "Ec"), lty = c(1,2),
           lwd=c(2,2), pch =1:2, 
           col = c(colors.EcA[6], colors.Ec[6]), cex = 1.5,
           xjust=0.5, yjust=0.5)
}
...
pdf(file="MothersPupsLog_v2.pdf", width = 8, height = 10)
layout(matrix(c(1,2), nrow=2), widths = c(6,6),
       heights = c(4,4))
p1.mothers.log(legend=FALSE)
p1.pups.log.v2(legend=TRUE)
dev.off()
